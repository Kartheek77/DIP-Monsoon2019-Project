...
def objectiveFunction(wfull, image1, cumulImageProbDensity):
	...
	for i in range(0,3):
		...
		if(0):
			...
			transformedImage = pixelVal_vec(image, lowerLimit, 0.0, upperLimit, 255.0)
...
def EnhanceTheImage(GivenImage):
	...
	lowerBound = 0.0
	upperBound = 0.5
	bounds = [(lowerBound,upperBound)]*3 
	print('starting the DEAlgo')
	result = differential_evolution(objectiveFunction, bounds,args = (GivenImage,cumulImageProbDensity1),disp = True,strategy = 'best1bin',tol=1e-4,popsize=40,mutation=0.8,recombination = 0.7,maxiter = 1000,workers = -1) 
...
temp1 = EnhanceTheImage(GivenImage[:50,:50,:])
temp2 = EnhanceTheImage(GivenImage[:50,50:100,:])
temp3 = EnhanceTheImage(GivenImage[:50,100:150,:])

temp10 = EnhanceTheImage(GivenImage[50:100,:50,:])
temp20 = EnhanceTheImage(GivenImage[50:100,50:100,:])
temp30 = EnhanceTheImage(GivenImage[50:100,100:150,:])

temp100 = EnhanceTheImage(GivenImage[100:150,:50,:])
temp200 = EnhanceTheImage(GivenImage[100:150,50:100,:])
temp300 = EnhanceTheImage(GivenImage[100:150,100:150,:])



row1 = np.concatenate((temp1,temp2,temp3),axis=1)
row2 = np.concatenate((temp10,temp20,temp30),axis=1)
row3 = np.concatenate((temp100,temp200,temp300),axis=1)
temp = np.concatenate((row1,row2,row3),axis=0)
