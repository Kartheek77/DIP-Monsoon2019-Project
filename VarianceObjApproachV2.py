...
def objectiveFunction(wfull,image1, cumulImageProbDensity,OriginalProbDensity):
	...
	for i in range(0,3):
		...
		if(1):
			...
			#transformedIntensities = range(0,256)
			#transformedIntensities = pixelVal_vec(range(0,256), lowerLimit, wfull[i+3], upperLimit, wfull[i+6])
...
def contrastStretching(image, w, s1, s2):
	...
	if(1):
		pixelVal_vec = np.vectorize(pixelVal) 
		# Apply contrast stretching.
		print('lowerLimit and upperLimit,s1,s2 in this fun call is  ',w,lowerLimit,upperLimit,s1,s2)
	...
	if(0):
		...
		NumberofPixelsPerIntensity = np.zeros(256)
		for j in range(0, 256):
			NumberofPixelsPerIntensity[j] = np.sum(transformedImage == j)
		probDensity = NumberofPixelsPerIntensity/TotalNumberOfPixelsInImage
		#plt.plot(np.arange(0,256,1),probDensity, color='red')
		#plt.fill_between(np.arange(0,256,1),probDensity, color='red')
		#plt.show()
...
def EnhanceTheImage(GivenImage):
	...
	result = differential_evolution(objectiveFunction, bounds,args = (GivenImage,cumulImageProbDensity1,probDensity1),disp = True,strategy = 'best1bin',tol=1e-4,popsize=40,mutation=0.8,recombination = 0.7,maxiter = 100,workers = 1) 
...
#GivenImage = cv2.imread("a13.jpg")
#GivenImage = cv2.imread("UW1.png")
GivenImage = cv2.imread("21.jpg")
