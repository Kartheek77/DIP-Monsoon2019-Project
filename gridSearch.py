...
def EnhanceTheImage(GivenImage,tempMutation,tempRecombination):
	...
	result = differential_evolution(objectiveFunction, bounds,args = (GivenImage,cumulImageProbDensity1),disp = True,strategy = 'best1bin',tol=0.0001,popsize=40,mutation=tempMutation,recombination = tempRecombination,maxiter = 100,workers = 10,updating= 'immediate') 
...
for i in range(1,10):
	for j in range(1,10):
		tempMutation = i/10
		tempRecombination = j/10		
		temp = EnhanceTheImage(tempGivenImage,tempMutation,tempRecombination)
		ObjValues[i,j] = objFunctionValue(temp)[0]
		print('mutation,recombination values are ',tempMutation,tempRecombination)
		print('ObjValue is ',ObjValues[i,j])

np.savetxt("gridSearch.csv", ObjValues, delimiter=",")

print('ObjFuncValueForEachImage')
print(ObjValues)
